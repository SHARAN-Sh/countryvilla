        ...
        <div class="example-code">
            <form action="" class="col-md-12 center-margin" id="articles_frm">
                ...
                <div class="form-row add-image">
                    ...
                    <!-- Upload user image preview -->
                    <div id="preview_Image"><input type="hidden" name="imageArrayname1[]"/></div>
                    <?php
                    if (!empty($articlesInfo->multi_image)) {
                        $imgRec = unserialize($articlesInfo->multi_image);
                        if (is_array($imgRec)) {
                            foreach ($imgRec as $key => $recimg) {
                                $deleteid = rand(0, 99999);
                                $imagePath = SITE_ROOT . 'images/mservices/images/' . $recimg;
                                if (file_exists($imagePath)) { ?>
                            ...
                            <span class="float-left">
                                <?php
                                if (file_exists(SITE_ROOT . "images/mservices/images/" . $recimg)):
                                    $filesize = filesize(SITE_ROOT . "images/mservices/images/" . $recimg);
                                    echo 'Size : ' . getFileFormattedSize($filesize);
                                endif;
                                ?>
                            </span>
                                                <a class="btn small float-right" href="javascript:void(0);"
                                                   onclick="deleteSavedArticlesimage(1<?php echo $deleteid; ?>);">
                                                    <i class="glyph-icon icon-trash-o"></i>
                                                </a>
                                            </div>
                                            <img src="<?php echo IMAGE_PATH . 'mservices/images/thumbnails/' . $recimg; ?>"
                                                 style="width:100%"/>
                                            <input type="hidden" name="imageArrayname1[]" value="<?php echo $recimg; ?>"
                                                   class="validate[required,length[0,250]]"/>
                                        </div>
                                    </div>
                                <?php }
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('#gallery_upload1').uploadify({
                ...
                'formData': {
                    PROJECT: '<?php echo SITE_FOLDER;?>',
                    targetFolder: 'images/mservices/images/',
                    thumb_width: 200,
                    thumb_height: 200
                },
                'method': 'post',
                ...
                'onUploadSuccess': function (file, data, response) {
                    $('#uploadedImageName').val('1');
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/mservices/uploaded_image_m.php', {imagefile: filename}, function (msg) {

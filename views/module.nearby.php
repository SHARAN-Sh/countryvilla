...
if (defined('NEARBY')) {
    ...
    if (!empty($recRows)) {
        ...
        foreach($recRows as $k=>$recRow){
            ...
            if($k==0){
                $nearbydetail_modals.='
                                    <iframe src="'. strip_tags($recRow->content) .'" 
                                        id="iframe_map" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                   
                                        ';
            }
            $nearbydetail.='
                                <li class="py-1 border-bottom getDirection" data-src="'. strip_tags($recRow->content) .'">' . $recRow->sub_title . '<p>' . $recRow->distance . '</p></li>
